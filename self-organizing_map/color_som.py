"""
    Self-organizing map (SOM) or Kohonen Network. Performs dimensionality
    reduction (3D RGB color space to 2D XY space). Serves as an unsupervised
    classifier.
"""
...
class SOM(nn.Module):
    """
    2-D Self-Organizing Map with Gaussian neighboorhood function
    ...
    def map_vects(self, input_vects):
        """
            For each training sample, compute the Euclidean distance between
            the sample and the weights for each neuron in the lattice. Returns
            a list of neurons which correspond with the BMUs for each sample.
        """
